...
module.exports = function (Votes) {
    ...
    Votes.getVotePosts = function (vid, set, start, end, uid, reverse, callback) {
        callback = callback || function () {
        };
        async.parallel({
            posts: function (next) {
                posts.getPostsByTid(vid, set, start, end, uid, reverse, next);
            },
            postCount: function (next) {
                Votes.getVoteField(vid, 'postcount', next);
            }
    ...
    Votes.addPostData = function (postData, uid, callback) {
        ...
        async.parallel({
            ...
            userData: function (next) {
                ...
                for (var i = 0, dataLength = postData.length; i < dataLength; i++) {
            ...
            privileges: function(next) {
                require('../privileges').posts.get(pids, uid, next);
            }
        }, function (err, results) {
            if (err) {
                return callback(err);
    ...
    Votes.increaseViewCount = function (vid, callback) {
        incrementFieldAndUpdateSortedSet(vid, 'viewcount', 1, 'votes:view', callback)
    };
